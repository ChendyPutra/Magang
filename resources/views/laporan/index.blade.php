...
<body class="bg-white ">
    ...
    <div style="display: flex; padding-top: 4rem;">
        <!-- Sidebar -->
        <aside
            style="width: 16rem; background-color: #E6ECFF; color: white; flex-shrink: 0; height: calc(100vh - 4rem); padding: 1rem;">
        ...
        <main style="flex: 1; padding: 1.5rem; background-color: #f3f4f6;">
            <div class="flex items-center space-x-6 md:space-x-10">
                <a href="#" id="hari-btn"
                    class="text-lg font-semibold text-navy hover:bg-gray-500 hover:text-white px-4 py-2 rounded transition duration-200">Rekap
                    Per-Hari</a>
                <a href="#" id="tahunan-btn"
                    class="text-lg font-semibold text-navy hover:bg-gray-500 hover:text-white px-4 py-2 rounded transition duration-200">Rekap
                    Bulanan-Tahunan</a>
                <a href="#" id="keuangan-btn"
                    class="text-lg font-semibold text-navy hover:bg-gray-500 hover:text-white px-4 py-2 rounded transition duration-200">Neraca
                    Keuangan</a>
            </div>
            ...
            <div id="rekap-tahunan" style="margin-top: 45px">
                <h2 class="text-xl font-bold text-center">Rekap Tahun {{ $tahunDipilih }}</h2>
                <!-- Filter Tahun -->
                <div class="flex justify-end mb-4">
                    <form method="GET" action="{{ route('laporan.index') }}">
                        <label for="tahun" class="mr-2 font-semibold">Filter Tahun:</label>
                        <select name="tahun" id="tahun" onchange="this.form.submit()"
                            class="px-3 py-2 text-sm border border-gray-300 rounded">
                            @foreach($tahunList as $year)
                                <option value="{{ $year }}" {{ $tahunDipilih == $year ? 'selected' : '' }}>{{ $year }}
                                </option>
                            @endforeach
                        </select>
                    </form>
                ...
                <table
                    style="width: 100%; margin-top: 1.5rem; background-color: white; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); border-radius: 0.375rem;">
                    <thead>
                        <tr style="background-color: #54BDFF; color: white;">
                            <th class="border-r" style="padding: 0.5rem;">No</th>
                            <th class="border-r" style="padding: 0.5rem;">Tanggal</th>
            ...
            <div id="neraca-keuangan" style="display: none; margin-top: 45px;">
                <h2 class="text-xl font-bold text-center">Neraca Keuangan Tahun {{ $tahunDipilih }}</h2>

                <div class="flex items-center justify-end mb-6">
                    <div class="flex space-x-2" style="margin-right: 10px">
                        <a href="{{ route('download.pdf', ['tahun' => $tahunDipilih]) }}" id="download-pdf"
                            class="px-4 py-2 text-white bg-gray-800 rounded-md hover:bg-purple-600">
                            <i class="mr-2 fas fa-download"></i> Download PDF
                        </a>

                    </div>
                    <!-- Filter Tahun -->
                    <form method="GET" action="{{ route('laporan.index') }}" class="flex justify-end mt-6 mb-2">
                    ...
                    <div class="p-6">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="flex justify-between">
                                <div>Pemasukan</div>
                                <div>Rp. {{ number_format($totalPemasukanNeraca, 0, ',', '.') }}</div>
                            </div>
                            <div class="flex justify-between">
                                <div>Pengeluaran</div>
                                <div>Rp. {{ number_format($totalPengeluaranNeraca, 0, ',', '.') }}</div>
                            </div>
                            <div class="flex justify-between font-bold">
                                <div>JUMLAH APBD</div>
                                <div>Rp. {{ number_format($totalKeseluruhanNeraca, 0, ',', '.') }}</div>
                            </div>
                        </div>
