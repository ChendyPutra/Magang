        ...
        <div class="p-6 bg-white rounded-lg ">
            ...
            <div class="flex items-center justify-around mb-6">
                ...
                <div class="flex space-x-2">
                    <form method="GET" action="{{ route('pengeluaran_view') }}" class="flex space-x-2 mb-6 justify-end">
                        <select name="bulan" class="px-3 py-2 text-sm border border-gray-300 rounded">
                            @foreach(range(1, 12) as $b)
                                <option value="{{ str_pad($b, 2, '0', STR_PAD_LEFT) }}" {{ $bulan == str_pad($b, 2, '0', STR_PAD_LEFT) ? 'selected' : '' }}>
                                    {{ date('M', mktime(0, 0, 0, $b, 1)) }}
                                </option>
                            @endforeach
                        </select>
                        <select name="tahun" class="px-3 py-2 text-sm border border-gray-300 rounded">
                            @foreach(range(date('Y'), date('Y') - 5) as $t)
                                <option value="{{ $t }}" {{ $tahun == $t ? 'selected' : '' }}>{{ $t }}</option>
                            @endforeach
                        </select>
                        <button type="submit" class="px-4 py-2 text-white bg-blue-600 rounded">Tampilkan</button>
                    </form>
        ...
        <table class="w-full max-w-5xl mx-auto border border-gray-400 table-auto">
            <thead>
                <tr>
                    ...
                    <th class="px-4 py-2 text-center text-white bg-red-700 border-b border-r border-gray-400">Sumber
                        Dana</th>
                    <th class="px-4 py-2 text-center text-white bg-red-700 border-b border-r border-gray-400">Jumlah
                    </th>
                    <th class="px-4 py-2 text-center text-white bg-red-700 border-b border-r border-gray-400">Tanggal
                    </th>
                    <th class="px-4 py-2 text-center text-white bg-red-700 border-b border-r border-gray-400">Keterangan
                    </th>
                </tr>
            </thead>
    ...
    <section class="py-8 bg-gray-100">
        <div class="container mx-auto">
            <div class="p-6 bg-white rounded-lg shadow-md">
                ...
                <div class="space-y-4">
                    <form action="{{ route('komentar.store', ['type' => 'pengeluaran']) }}" method="POST"
                        class="space-y-4">
                        @csrf
                        <div>
