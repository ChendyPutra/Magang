...
@section('content')
    ...
    <!-- Tabel Pemasukan -->
    <div id="pemasukan-table" style="display: block;">
        <h2 class="text-xl font-bold text-center mb-4">Tabel Pemasukan</h2>
        <button onclick="toggleModal('modal-tambah-pemasukan')" class="px-4 py-2 text-white bg-purple-700 rounded">
            Tambah Pemasukan
        </button>

        <!-- Modal Tambah Pemasukan -->
        <div id="modal-tambah-pemasukan" class="fixed inset-0 z-50 hidden overflow-y-auto bg-gray-800 bg-opacity-50">
            <div class="flex items-center justify-center min-h-screen">
                <div class="p-6 bg-white rounded-lg shadow-lg w-96">
                    <h2 class="mb-4 text-lg font-bold text-center">Tambah Pemasukan</h2>
                    <form action="{{ route('pemasukan.store') }}" method="POST">
                        ...
                        <div class="flex justify-end space-x-2">
                            <button type="button" onclick="toggleModal('modal-tambah-pemasukan')" class="px-4 py-2 text-white bg-gray-400 rounded">Batal</button>
            ...
            <tbody>
                @foreach($pemasukan as $index => $item)
                    <tr class="border-t">
                        ...
                        <td class="p-2">
                            <!-- Tombol Edit -->
                            <button onclick="toggleModal('modal-edit-pemasukan-{{ $item->id }}')" class="px-4 py-2 text-white bg-blue-500 rounded">
                                Edit
                            </button>

                            <!-- Modal Edit -->
                            <div id="modal-edit-pemasukan-{{ $item->id }}" class="fixed inset-0 z-50 hidden overflow-y-auto bg-gray-800 bg-opacity-50">
                                <div class="flex items-center justify-center min-h-screen">
                                    <div class="p-6 bg-white rounded-lg shadow-lg w-96">
                                        <h2 class="mb-4 text-lg font-bold text-center">Edit Pemasukan</h2>
                                        <form action="{{ route('pemasukan.update', $item->id) }}" method="POST">
                                            ...
                                            <div class="flex justify-end space-x-2">
                                                <button type="button" onclick="toggleModal('modal-edit-pemasukan-{{ $item->id }}')" class="px-4 py-2 text-white bg-gray-400 rounded">Batal</button>
                                                <button type="submit" class="px-4 py-2 text-white bg-purple-700 rounded">Simpan</button>
                                            </div>
                                        </form>
                                    </div>
                            ...
                            <!-- Tombol Hapus -->
                            <form action="{{ route('pemasukan.destroy', $item->id) }}" method="POST" style="display:inline;" onsubmit="return confirmDelete()">
                                @csrf
                                @method('DELETE')
